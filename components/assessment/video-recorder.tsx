...
import { Video, Mic, Pause, Square, ChevronDown } from "lucide-react";
    ...
    <div className="rounded-xl border-2 border-meti-slate/15 bg-white p-5 space-y-4">
      ...
      {showTranscript && (
        <div className="rounded-lg bg-meti-cream/50 p-3.5 text-xs text-meti-slate leading-relaxed">
          "Thanks for the time today. Our recommendation centers on three levers: first,
          consolidating fulfilment nodes to cut redundant handling costs; second, renegotiating
          last-mile carrier contracts against updated volume tiers; third, phasing a 12-week
          pilot before full rollout to de-risk the transition…"
        </div>
      )}
    </div>
  );
